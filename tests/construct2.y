...
%error-verbose
%lex-param {construct2::ParseDriver* parser}
%locations
%define api.namespace {construct2}
%parse-param {construct2::ParseDriver* parser}
%parse-param {construct2::AstNode** result}
...
A* A::parse(istream& input, ostream& errors) {
  return dynamic_cast<A*>(construct2::parse(input, errors));
}
...
B::B(ParseState parseState, AttributeMap& keyword_args)
  ...
  if (!(keyword_args.Take("a1", a1))) {
    a1 = new D(defaultParseState, /*str*/ "blank");
  }
}
...
void B::format(ostream& out, int precedence) {
  out << "x";
  out << " ";
  if (a1 != nullptr && (a1->className() != "D" || dynamic_cast<D*>(a1)->str != "blank")) {
...
int ParseSamples() {
  int num_errors = 0;
/* BEGIN SAMPLES */
  num_errors += ParseSample<A>(R"#A#(())#A#", kFail);
  num_errors += ParseSample<A>(R"#A#(x y)#A#", R"#A#((B a1:(D str:blank)))#A#");
  num_errors += ParseSample<A>(R"#A#(x foo y)#A#", R"#A#((B a1:(D str:foo)))#A#");
/* END SAMPLES */
...
const char usage[] = "usage: construct2 [input-file | --samples]\n";
...
int main(int argc, char** argv) {
  ...
  if (argc == 2 && std::string(argv[1]) == "--samples") {
    if (construct2::ParseSamples() > 0) exit(1);
  } else {
    ifstream file;
    if (argc == 2) {
      file.open(argv[1]);
    ...
    construct2::ParseAndPrint(input, std::cout);
  }
  return 0;
}
