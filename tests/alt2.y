...
%error-verbose
%lex-param {alt2::ParseDriver* parser}
%locations
%define api.namespace {alt2}
%parse-param {alt2::ParseDriver* parser}
%parse-param {alt2::AstNode** result}
...
A* A::parse(istream& input, ostream& errors) {
  return dynamic_cast<A*>(alt2::parse(input, errors));
}
...
const char usage[] = "usage: alt2 [input-file | --samples]\n";
...
int main(int argc, char** argv) {
  ...
  if (argc == 2 && std::string(argv[1]) == "--samples") {
    ...
    alt2::ParseAndPrint(input, std::cout);
  }
  return 0;
}
