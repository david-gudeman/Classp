...
%error-verbose
%lex-param {construct1::ParseDriver* parser}
%locations
%define api.namespace {construct1}
%parse-param {construct1::ParseDriver* parser}
%parse-param {construct1::AstNode** result}
...
A* A::parse(istream& input, ostream& errors) {
  return dynamic_cast<A*>(construct1::parse(input, errors));
}
...
/* BEGIN SAMPLES */
  ...
  num_errors += ParseSample<A>(R"#A#(x y)#A#", kFail);
...
const char usage[] = "usage: construct1 [input-file | --samples]\n";
...
int main(int argc, char** argv) {
  ...
  if (argc == 2 && std::string(argv[1]) == "--samples") {
    if (construct1::ParseSamples() > 0) exit(1);
  } else {
    ifstream file;
    if (argc == 2) {
      file.open(argv[1]);
    ...
    construct1::ParseAndPrint(input, std::cout);
  }
  return 0;
}
